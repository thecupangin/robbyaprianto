<div>
        ...
        <form wire:submit.prevent="onCreateDatabase">
            ...
            <div class="form-group mb-3">
                <label class="form-label" for="purchase_code">{{ __('Purchase code') }}</label>
                <input placeholder="Enter random value" type="text" wire:model="purchase_code" id="purchase_code" class="form-control @error('purchase_code') is-invalid @enderror" required />
            </div>
            ...
            <div class="col-md-12 text-center">
                ...
                @if ( $continue == true )
                    <a class="btn bg-gradient-success mt-3 mb-0" href="{{ route('sw_admin') }}">{{ __('Continue') }}</a>
                @endif
