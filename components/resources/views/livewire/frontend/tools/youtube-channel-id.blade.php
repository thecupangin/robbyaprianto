<div>
      ...
      <form wire:submit.prevent="onYoutubeChannelId">
            ...
            <div class="col-auto">
                <div class="form-group">
                    <button class="btn bg-gradient-info mx-auto d-block" wire:loading.attr="disabled">
                        <span>
                            <div wire:loading.inline wire:target="onYoutubeChannelId">
                                <x-loading />
                            </div>
                            <span wire:target="onYoutubeChannelId">{{ __('Find now') }}</span>
                        </span>
                    </button>
                </div>
            </div>
        ...
        @if ( !empty($data) )  
            <div class="text-center mt-3" wire:loading.remove wire:target="onYoutubeChannelId">
                <div class="alert alert-important alert-success">
                    <strong>{{ __('Channel ID') }}:</strong> {{ $data }}
                </div>
            </div>
        @endif
