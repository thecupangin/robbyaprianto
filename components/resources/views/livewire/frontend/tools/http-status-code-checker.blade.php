<div>
      ...
      <form wire:submit.prevent="onHttpStatusCodeChecker">
            ...
            <div class="col-auto">
                <div class="form-group">
                    <button class="btn bg-gradient-info mx-auto d-block" wire:loading.attr="disabled">
                        <span>
                            <div wire:loading.inline wire:target="onHttpStatusCodeChecker">
                                <x-loading />
                            </div>
                            <span wire:target="onHttpStatusCodeChecker">{{ __('Check') }}</span>
                        </span>
                    </button>
                </div>
            </div>
                   ...
                   <tbody>
                           <tr>
                               ...
                               <td class="border">
                                    @switch($data['status'])
                                        @case(200)
                                             <span class="text-success text-bold">{{ $data['status'] }}</span>
                                            @break
                                        @case(301)
                                        @case(302)
                                             <span class="text-warning text-bold">{{ $data['status'] }}</span>
                                            @break
                                    
                                        @default
                                                <span class="text-danger text-bold">{{ $data['status'] }}</span>
                                    @endswitch
                                </td>
                           </tr>
                   </tbody>
