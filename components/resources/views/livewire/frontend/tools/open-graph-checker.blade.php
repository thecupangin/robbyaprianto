<div>
      ...
      <form wire:submit.prevent="onOpenGraphChecker">
            ...
            <div class="col-auto">
                <div class="form-group">
                    <button class="btn bg-gradient-info mx-auto d-block" wire:loading.attr="disabled">
                        <span>
                            <div wire:loading.inline wire:target="onOpenGraphChecker">
                                <x-loading />
                            </div>
                            <span wire:target="onOpenGraphChecker">{{ __('Check') }}</span>
                        </span>
                    </button>
                </div>
            </div>
                ...
                <table class="table table-bordered border">
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $key => $value)
                            <tr>
                                <td class="bg-gradient-success text-white">{{ $key }}</td>
                                <td>
                                    {{ $value }}
                                    @if ($key == 'og:image')
                                       <p class="mt-3"><img src="{{ $value }}" class="img-thumbnail" alt="og:image"></p>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
