					...
					<form wire:submit.prevent="addToMenu">
							...
							<div class="form-group mb-3">
								...
								<select class="form-control" wire:model="class">
									...
									<optgroup label="{{ __('Outline Buttons') }}">
										<option value="bg-gradient-outline-default">{{ __('Default') }}</option>
										<option value="bg-gradient-outline-primary">{{ __('Primary') }}</option>
										<option value="bg-gradient-outline-secondary">{{ __('Secondary') }}</option>
										<option value="bg-gradient-outline-info">{{ __('Info') }}</option>
										<option value="bg-gradient-outline-success">{{ __('Success') }}</option>
										<option value="bg-gradient-outline-danger">{{ __('Danger') }}</option>
										<option value="bg-gradient-outline-warning">{{ __('Warning') }}</option>
										</optgroup>
								</select>
							</div>
						...
						<div class="form-group">
							<button class="btn bg-gradient-info float-end">
								<span>
									<div wire:loading wire:target="addToMenu">
										<x-loading />
									</div>
		...
		<div class="col-12 col-md-6">
			<div class="card">
				<div class="card-body">
                    ...
                    <div class="col-auto">
                        <button class="btn bg-gradient-primary float-end mt-3" id="onUpdateMenu" wire:loading.attr="disabled">
                          <span>
                            <div wire:loading.inline wire.emit.target="onUpdateMenu">
                              <x-loading />
                            </div>
