...
RewriteRule ^' . $arr['rule'] . '$ '. substr($arr['out'],0,-1) . ' [L]';
        ...
        public function getType1()
        {
            ...
            if( !empty($this->urlArr['port']) )
                $arr['expl'] = $this->urlArr['host'].':'.$this->urlArr['port'].$this->newpath1. $rewriteSpl;
            else
                $arr['expl'] = $this->urlArr['host'].$this->newpath1. $rewriteSpl;
            if($this->urlArr['scheme'] != '')
                $arr['expl'] = $this->urlArr['scheme'].'://'.$arr['expl'];
